...
export interface ExcelRaceData {
  raceName: string;
  raceDate: string;
  raceType: 'montagne' | 'rallye' | 'karting' | 'acceleration';
  results: Array<{
    position: number;
    driverName: string;
    points: number;
...
export const parseExcelFile = async (file: File, raceType: 'montagne' | 'rallye' | 'karting' | 'acceleration'): Promise<ExcelRaceData[]> => {
