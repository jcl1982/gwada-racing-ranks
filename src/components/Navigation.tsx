        ...
        <nav className="flex flex-nowrap items-center gap-1 md:gap-2 text-[11px] sm:text-xs md:text-sm overflow-x-auto flex-1 min-w-0 -mx-1 px-1 scrollbar-thin">
          {visibleOtherItems.map(({ id, label, icon: Icon }) => (
            <button 
              ...
              aria-label={label} 
              className={`${btnBase} ${currentView === id ? activeClass : inactiveClass}`}
            >
          ...
          <button 
            ...
            aria-label="Championnat Rallye - Montagne" 
            className={`${btnBase} ${isRallyeMontagnView ? activeClass : inactiveClass}`}
          >
          ...
          <button 
            onClick={() => onViewChange('acceleration')} 
            aria-label="Championnat Accélération" 
            className={`${btnBase} ${currentView === 'acceleration' ? activeClass : inactiveClass}`}
          >
          ...
          <button 
            onClick={() => onViewChange('karting')} 
            aria-label="Championnat Karting" 
            className={`${btnBase} ${currentView === 'karting' ? activeClass : inactiveClass}`}
          >
