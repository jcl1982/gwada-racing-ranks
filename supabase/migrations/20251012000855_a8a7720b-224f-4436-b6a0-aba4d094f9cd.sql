-- Correction: Ne sauvegarder que les pilotes avec position valide pour le type
...
BEGIN
  ...
  -- CRITICAL FIX: Only save drivers that have a valid position for the current standing_type
  ...
  FROM new_positions
  WHERE CASE p_standing_type
    WHEN 'general' THEN general_position IS NOT NULL
    WHEN 'montagne' THEN montagne_position IS NOT NULL
    WHEN 'rallye' THEN rallye_position IS NOT NULL
    WHEN 'c2r2' THEN c2r2_position IS NOT NULL
  END;
  
  RAISE NOTICE 'Classement % sauvegardé pour le championnat % (tous les pilotes avec positions préservées)', p_standing_type, p_championship_id;
END;
$function$;